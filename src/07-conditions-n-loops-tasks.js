...
function isCreditCardNumber(/* ccn */) {
  // const arr = [];
  // const card = ccn.toString();
  // for (let i = 0; i < card.length; i += 1) {
  //   if (i % 2 === 0) {
  //     const m = parseInt(card[i]) * 2;
  //     if (m > 9) {
  //       arr.push(m - 9);
  //     } else {
  //       arr.push(m);
...
function getCommonDirectoryPath(pathes) {
  ...
  const minCount = str.reduce((acc, el) => (el.length > acc ? acc : el.length), str[0].length);
  let res = '';
  for (let i = 0; i < minCount; i += 1) {
    const ddd = str.map((s) => `${s.slice(0, i + 1).join('/')}${minCount === i + 1 ? '' : '/'}`);
    const asdf = ddd[0];
    if (ddd.every((d) => asdf === d)) {
      res = asdf;
    } else {
      return res;
    }
...
function evaluateTicTacToePosition(position) {
  for (let i = 0; i < 3; i += 1) {
    const firstChar = position[i][0];
    if (firstChar !== undefined) {
      if (position[i][1] === firstChar && position[i][2] === firstChar) {
        return firstChar;
      }
    }
  }
  for (let j = 0; j < 3; j += 1) {
    const firstChar = position[0][j];
    if (firstChar !== undefined) {
      if (position[1][j] === firstChar && position[2][j] === firstChar) {
        return firstChar;
      }
    }
  }
  const firstChar = position[0][0];
  if (firstChar !== undefined) {
    if (position[1][1] === firstChar && position[2][2] === firstChar) {
      return firstChar;
    }
  }
  const firstCharTwo = position[0][2];
  if (firstCharTwo !== undefined) {
    if (position[1][1] === firstCharTwo && position[2][0] === firstCharTwo) {
      return firstCharTwo;
    }
  }
  return undefined;
